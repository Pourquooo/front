...
<script>
import { getAllStudents } from '@/api/manager';
...
export default {
  ...
  methods: {
    ...
    modifyRow(index, rows){
      this.form = {
        ...
        userFace: rows[index].userFace? rows[index].userFace:''
      };
      this.dialogFormVisible = true;
    },
    getAllInfo(){
      let promise = getAllStudents();
      promise.then((res)=>{
        const {status} = res;
        if(status){
          const {data} = res
  ...
  handleClassChange() {
      ...
      switch (this.newForm.class) {
        case "1班":
          this.newForm.class = 2101;
          break;
        case "2班":
          this.newForm.class = 2102;
          break;
        case "3班":
          this.newForm.class = 2103;
          break;
        case "4班":
          this.newForm.class = 2104;
          break;
        case "5班":
          this.newForm.class = 2105;
          break;
        case "6班":
          this.newForm.class = 2106;
          break;
        default:
    ...
    updateInfor(){
      ...
      const banji = myForm.class ? myForm.class:this.form.class;
      ...
      let promise = altInform({userFace,userId,userName,email, 'class':banji, roleType, password});
      console.log(banji)
      promise.then((res)=>{
        const {status} = res;
        if(status){
